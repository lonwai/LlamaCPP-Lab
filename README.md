...
- 📊 **实时性能仪表盘**：动态展示生成速度、首字延迟 (TTFT)、Token 消耗统计与上下文衰减趋势
- 🗄️ **纯文件化数据管理**：对话记录、参数预设、基准测试日志自动落盘为 JSON，支持一键导入/导出，零数据库依赖
- 🔧 **调优辅助工具**：内置采样参数预设、原始 API 响应调试面板、批量测试记录对比
- 🖥️ **轻量开箱即用**：React 前端 + 极简 Node 代理层，完美兼容 Windows/macOS/Linux，双击即可运行
...
- [ ] Phase 2: 参数预设管理 + Recharts 趋势图 + 会话导出
- [ ] Phase 3: 上下文衰减分析 + 批量测试对比报告
- [ ] Phase 4: Electron/Tauri 桌面版打包
